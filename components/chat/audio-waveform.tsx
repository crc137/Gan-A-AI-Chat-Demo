...
export function AudioWaveform({ isRecording, stream }: AudioWaveformProps) {
  ...
  useEffect(() => {
    if (!isRecording || !stream) {
      setLevels(Array(24).fill(0).map(() => 0.05 + Math.random() * 0.1))
      historyRef.current = []
      return
    }
    ...
    const updateLevels = () => {
      ...
      if (historyRef.current.length > 12) {historyRef.current.pop()}
  ...
  return (<div className="flex items-center justify-center gap-[3px] h-8 flex-1 px-2">{levels.map((level, index) => (<div key={index}className="bg-zinc-400 rounded-full transition-all duration-75"style={{width: "3px",height: `${Math.max(4, level * 28)}px`}}/>))}</div>)
}
