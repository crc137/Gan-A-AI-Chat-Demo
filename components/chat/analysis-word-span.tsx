...
interface AnalysisWordSpanProps {word: string}
...
export function AnalysisWordSpan({ word }: AnalysisWordSpanProps) {
  ...
  useEffect(() => {
    ...
    const animateBlur = (currentTime: number) => {
      ...
      if (progress < 1) {animationRef.current = requestAnimationFrame(animateBlur)} else {setAnimationComplete(true)}
    }
  ...
  return (<span className="inline"style={{filter: animationComplete ? "none" : `blur(${blurAmount}px)`,opacity: animationComplete ? 1 : opacity}}>{word}</span>)
}
